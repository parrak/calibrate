  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden">
        <div className="mx-auto max-w-6xl px-6 py-24 md:py-32 grid md:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <p className="mt-6 text-xl text-[color:var(--mute)] max-w-xl leading-relaxed">
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-4">
              ...
              <a
                href="/early-access"
                className="inline-flex items-center justify-center px-6 py-3 rounded-lg border-2 border-[color:var(--border)] text-[color:var(--fg)] font-semibold hover:bg-neutral-50 hover:border-[color:var(--fg)]/20 transition-all"
              >
                Join Early Access
              </a>
            </div>
            <p className="mt-3 text-sm text-[color:var(--mute)]">
              Questions? <a href="mailto:contact@calibr.lat" className="underline hover:text-[color:var(--fg)]">Contact us</a>
            </p>
            <p className="mt-6 text-sm text-[color:var(--mute)] flex items-center gap-2">
        ...
        <div className="absolute -z-10 inset-0 pointer-events-none [mask-image:radial-gradient(ellipse_at_center,black,transparent_70%)]">
          <div className="absolute -top-40 -right-40 w-[580px] h-[580px] rounded-full bg-[color:var(--brand)]/10 blur-3xl" />
      ...
      <section className="mx-auto max-w-6xl px-6 py-8">
        <div className="rounded-xl border border-[color:var(--border)] bg-white/40 backdrop-blur-sm p-8">
          <div className="grid md:grid-cols-3 gap-8">
            <blockquote className="text-sm text-[color:var(--mute)] leading-relaxed">
              <p className="mb-3">"Finally stopped editing CSVs for every sale."</p>
              <cite className="text-[color:var(--fg)] font-medium not-italic">— Shopify Merchant</cite>
            </blockquote>
            <blockquote className="text-sm text-[color:var(--mute)] leading-relaxed">
              <p className="mb-3">"Guardrails let us automate without fear."</p>
              <cite className="text-[color:var(--fg)] font-medium not-italic">— Agency Owner</cite>
            </blockquote>
            <blockquote className="text-sm text-[color:var(--mute)] leading-relaxed">
              <p className="mb-3">"Explainable AI made approvals a breeze."</p>
              <cite className="text-[color:var(--fg)] font-medium not-italic">— DTC Brand</cite>
            </blockquote>
          </div>
        </div>
      </section>
      ...
      <Section id="how" title="How it works">
        <div className="relative">
          <div className="hidden md:block absolute top-16 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-[color:var(--border)] to-transparent"></div>
...
function Feature({ icon, title, desc }: { icon: string; title: string; desc: string }) {
  return (
    <div className="group rounded-xl border border-[color:var(--border)] bg-white p-6 hover:border-[color:var(--brand)]/50 hover:shadow-lg transition-all duration-300">
      <div className="text-3xl mb-3 group-hover:scale-110 transition-transform">{icon}</div>
      <div className="text-lg font-semibold mb-2">{title}</div>
      <p className="text-sm text-[color:var(--mute)] leading-relaxed">{desc}</p>
    </div>
  )
}
...
function Step({ n, title, children }: { n: string; title: string; children: React.ReactNode }) {
  return (
    <li className="relative rounded-xl border border-[color:var(--border)] bg-white p-6 hover:border-[color:var(--brand)]/30 hover:shadow-md transition-all">
      <div className="absolute -top-3 left-6 w-8 h-8 rounded-full bg-[color:var(--brand)] text-white font-bold flex items-center justify-center text-sm shadow-lg">
        {n}
      </div>
      <div className="text-lg font-semibold mt-2 mb-2">{title}</div>
      <p className="text-sm text-[color:var(--mute)] leading-relaxed">{children}</p>
    </li>
  )
}
