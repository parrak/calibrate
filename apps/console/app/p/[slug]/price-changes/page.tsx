...
type Item = {
  ...
  context?: any
  source?: string
  policyResult?: { ok: boolean; checks: any[] }
}
...
export default function ProjectPriceChanges({ params }: { params: { slug: string } }) {
  ...
  const [status, setStatus] = useState<
    'PENDING' | 'APPROVED' | 'APPLIED' | 'REJECTED' | 'FAILED' | 'ROLLED_BACK' | 'ALL'
  >('PENDING')
  ...
  async function load() {
    ...
    try {
      ...
      else setError('Failed to load price changes')
          ...
          return (
            <tr key={i.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="px-4 py-3 text-gray-700">
                {fmt(i.currency, i.fromAmount)} → {fmt(i.currency, i.toAmount)}
              </td>
              ...
              <td className="px-4 py-3 text-right space-x-2">
                <Button variant="ghost" onClick={() => { setActive(i); setOpen(true) }}>
                  View
                </Button>
                <Button variant="ghost" onClick={() => act(i.id, 'approve')}>
                  Approve
                </Button>
