...
| 3 | OAuth callback tests only covered missing parameters. | HMAC regressions or callback routing issues could ship unchecked. | Added Vitest coverage for invalid signatures and the successful token-exchange path, including assertions for internal fetch targets. |
