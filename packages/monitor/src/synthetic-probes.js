 ...
 */
export async function executeProbe(config, baseUrl) {
 ...
 * Execute all probes
 */
export async function executeAllProbes(baseUrl, probes = DEFAULT_PROBES) {
    const results = await Promise.all(probes.map(probe => executeProbe(probe, baseUrl)));
 ...
 */
export function recordProbeResult(result) {
 ...
 */
export function getProbeStats(probeId) {
 ...
 */
export function getAllProbeStats() {
    const stats = new Map();
 ...
 */
export function getAllProbeResults() {
    return new Map(probeResults);
}
/**
 * Clear probe results
 */
export function clearProbeResults(probeId) {
    if (probeId) {
 ...
 */
export function startPeriodicProbing(intervalMs = 5 * 60 * 1000, // 5 minutes default
baseUrl, probes) {
...
//# sourceMappingURL=synthetic-probes.js.map