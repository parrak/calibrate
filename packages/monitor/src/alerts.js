...
export const DEFAULT_ALERT_POLICIES = [
    // API Performance Alerts
    {
        ...
        message: (metrics) => `API p95 latency is ${metrics.performance.p95ResponseTime}ms (threshold: 1500ms)`,
    ...
    },
    {
        ...
        message: (metrics) => `🚨 CRITICAL: API p95 latency is ${metrics.performance.p95ResponseTime}ms (threshold: 3000ms)`,
    ...
    // Error Rate Alerts
    {
        ...
        message: (metrics) => `API error rate is ${metrics.performance.errorRate.toFixed(2)}% (threshold: 2%)`,
    ...
    },
    {
        ...
        message: (metrics) => `🚨 CRITICAL: API error rate is ${metrics.performance.errorRate.toFixed(2)}% (threshold: 5%)`,
        cooldownMs: 5 * 60 * 1000, // 5 minutes
        enabled: true
    },
    // Event Bus Alerts
    {
        ...
        message: (metrics) => `Event bus failure rate is ${metrics.eventBus.failureRate.toFixed(2)}% (threshold: 5%)`,
    ...
    },
    {
        ...
        message: (metrics) => `Event bus p95 latency is ${metrics.eventBus.p95Latency}ms (threshold: 2000ms)`,
        cooldownMs: 10 * 60 * 1000, // 10 minutes
        enabled: true
    },
    // Outbox / DLQ Alerts
    {
        ...
        message: (metrics) => `Dead letter queue has ${metrics.outbox.dlq} events (threshold: 10). Manual intervention may be required.`,
    ...
    },
    {
        ...
        message: (metrics) => `🚨 CRITICAL: Dead letter queue has ${metrics.outbox.dlq} events (threshold: 100). Immediate action required!`,
        cooldownMs: 15 * 60 * 1000, // 15 minutes
        enabled: true
    },
    {
        ...
        message: (metrics) => `Outbox has pending events older than ${Math.round(metrics.outbox.backlogAge / 1000 / 60)} minutes`,
        cooldownMs: 10 * 60 * 1000, // 10 minutes
        enabled: true
    },
    // Connector Health Alerts
    {
        ...
        severity: 'critical',
        channels: ['slack', 'pagerduty'],
        condition: (metrics) => metrics.connectors?.some((c) => c.status === 'down'),
        message: (metrics) => {
    ...
    },
    {
        ...
        severity: 'warning',
        channels: ['slack'],
        condition: (metrics) => metrics.connectors?.some((c) => c.status === 'degraded'),
        message: (metrics) => {
            const degradedConnectors = metrics.connectors
                .filter((c) => c.status === 'degraded')
                .map((c) => `${c.type} (${c.successRate.toFixed(1)}% success)`)
                .join(', ');
    ...
    },
    {
        ...
        severity: 'warning',
        channels: ['slack'],
        condition: (metrics) => metrics.connectors?.some((c) => c.totalOperations > 10 && c.successRate < 98),
        message: (metrics) => {
            const lowSuccessConnectors = metrics.connectors
                .filter((c) => c.totalOperations > 10 && c.successRate < 98)
                .map((c) => `${c.type} (${c.successRate.toFixed(1)}%)`)
                .join(', ');
    ...
    // Competitor Monitoring Alerts
    {
        ...
        message: (metrics) => `Competitor scraping error rate is ${metrics.competitorMonitor.errorRate.toFixed(2)}% (threshold: 1%) - ${metrics.competitorMonitor.totalErrors} errors in last 24h`,
        cooldownMs: 30 * 60 * 1000, // 30 minutes
        enabled: true
    },
    {
        ...
        message: (metrics) => `🚨 CRITICAL: Competitor scraping error rate is ${metrics.competitorMonitor.errorRate.toFixed(2)}% (threshold: 5%) - ${metrics.competitorMonitor.totalErrors} errors in last 24h`,
        cooldownMs: 15 * 60 * 1000, // 15 minutes
        enabled: true
    },
    {
        ...
        severity: 'warning',
        channels: ['slack'],
        condition: (metrics) => metrics.competitorMonitor?.failedCompetitors && metrics.competitorMonitor.failedCompetitors.length > 0,
        message: (metrics) => {
            const failed = metrics.competitorMonitor.failedCompetitors;
            const competitorNames = failed.map((c) => `${c.name} (${c.consecutiveFailures} failures)`).join(', ');
    ...
    },
    {
        ...
        severity: 'warning',
        channels: ['slack', 'email'],
        condition: (metrics) => metrics.competitorMonitor?.staleCompetitors && metrics.competitorMonitor.staleCompetitors.length > 0,
        message: (metrics) => {
            const stale = metrics.competitorMonitor.staleCompetitors;
            const competitorNames = stale.map((c) => `${c.name} (last checked: ${new Date(c.lastChecked).toLocaleString()})`).join(', ');
    ...
    // Cron Job Alerts
    {
        ...
        severity: 'critical',
        channels: ['slack', 'pagerduty'],
        condition: (metrics) => metrics.cronJobs?.missing && metrics.cronJobs.missing.length > 0,
        message: (metrics) => {
            const missing = metrics.cronJobs.missing;
            const jobNames = missing.map((j) => j.config.name).join(', ');
    ...
    },
    {
        ...
        severity: 'warning',
        channels: ['slack', 'email'],
        condition: (metrics) => metrics.cronJobs?.failed && metrics.cronJobs.failed.length > 0,
        message: (metrics) => {
            const failed = metrics.cronJobs.failed;
            const jobInfo = failed
                .map((j) => `${j.config.name} (${j.consecutiveFailures} failures)`)
                .join(', ');
    ...
    },
    {
        ...
        severity: 'warning',
        channels: ['slack'],
        condition: (metrics) => metrics.cronJobs?.unreliable && metrics.cronJobs.unreliable.length > 0,
        message: (metrics) => {
            const unreliable = metrics.cronJobs.unreliable;
            const jobInfo = unreliable
                .map((j) => `${j.config.name} (${j.reliability.toFixed(1)}% reliability)`)
                .join(', ');
    ...
    // Automation Runner Alerts (M1.6)
    {
        ...
        severity: 'warning',
        channels: ['slack', 'email'],
        condition: (metrics) => metrics.automation?.successRate !== undefined && metrics.automation.successRate < 97,
        message: (metrics) => `Rule application success rate is ${metrics.automation.successRate.toFixed(2)}% (threshold: 97%)`,
        cooldownMs: 15 * 60 * 1000, // 15 minutes
        enabled: true
    },
    {
        ...
        severity: 'critical',
        channels: ['slack', 'pagerduty'],
        condition: (metrics) => metrics.automation?.successRate !== undefined && metrics.automation.successRate < 90,
        message: (metrics) => `🚨 CRITICAL: Rule application success rate is ${metrics.automation.successRate.toFixed(2)}% (threshold: 90%)`,
        cooldownMs: 10 * 60 * 1000, // 10 minutes
        enabled: true
    },
    {
        ...
        severity: 'warning',
        channels: ['slack'],
        condition: (metrics) => metrics.automation?.dlqSize !== undefined && metrics.automation.dlqSize > 50,
        message: (metrics) => `Automation DLQ has ${metrics.automation.dlqSize} failed targets (threshold: 50)`,
        cooldownMs: 20 * 60 * 1000, // 20 minutes
        enabled: true
    },
    {
        ...
        severity: 'critical',
        channels: ['slack', 'pagerduty'],
        condition: (metrics) => metrics.automation?.dlqSize !== undefined && metrics.automation.dlqSize > 100,
        message: (metrics) => `🚨 CRITICAL: Automation DLQ has ${metrics.automation.dlqSize} failed targets (threshold: 100). Immediate action required!`,
        cooldownMs: 15 * 60 * 1000, // 15 minutes
        enabled: true
    },
    {
        ...
        severity: 'warning',
        channels: ['slack'],
        condition: (metrics) => metrics.automation?.rateLimitErrors !== undefined && metrics.automation.rateLimitErrors > 3,
        message: (metrics) => `Shopify rate limit burst detected: ${metrics.automation.rateLimitErrors} 429 errors in last 5 minutes`,
        cooldownMs: 10 * 60 * 1000, // 10 minutes
        enabled: true
    },
    {
        ...
        severity: 'warning',
        channels: ['slack'],
        condition: (metrics) => metrics.automation?.averageDuration !== undefined &&
            metrics.automation.averageDuration > 10 * 60 * 1000, // 10 minutes in ms
        message: (metrics) => `Automation run duration is ${(metrics.automation.averageDuration / 1000 / 60).toFixed(1)} minutes (threshold: 10 minutes for 100 SKUs)`,
        cooldownMs: 30 * 60 * 1000, // 30 minutes
        enabled: true
    }
];
 ...
 */
export function checkAlertPolicies(metrics, policies = DEFAULT_ALERT_POLICIES) {
    const now = Date.now();
 ...
 */
export function clearAlertCooldown(policyId) {
    if (policyId) {
