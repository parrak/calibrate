 ...
 */
export function recordEventMetric(metric) {
 ...
 */
export function getEventBusStats(timeRangeMs = 60 * 60 * 1000, // 1 hour default
eventType) {
    const now = Date.now();
 ...
 */
export function getSlowEvents(thresholdMs = 1000, timeRangeMs = 60 * 60 * 1000) {
    const now = Date.now();
    const startTime = now - timeRangeMs;
    const allMetrics = [];
    for (const metrics of eventMetrics.values()) {
 ...
 * Get failed events
 */
export function getFailedEvents(timeRangeMs = 60 * 60 * 1000) {
    const now = Date.now();
    const startTime = now - timeRangeMs;
    const allMetrics = [];
    for (const metrics of eventMetrics.values()) {
 ...
 */
export function getEventsByCorrelation(correlationId) {
    const allMetrics = [];
    for (const metrics of eventMetrics.values()) {
 ...
 */
export function getAllEventMetrics() {
