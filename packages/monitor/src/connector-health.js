...
const connectorHealthChecks = new Map();
// Configuration
 ...
 */
export function recordConnectorMetric(metric) {
 ...
 */
export function recordConnectorHealthCheck(healthCheck) {
 ...
 */
export function getConnectorHealthStats(connectorType, connectorId, timeRangeMs = 60 * 60 * 1000 // 1 hour default
) {
    const now = Date.now();
 ...
 */
export function getAllConnectorHealthStats(timeRangeMs = 60 * 60 * 1000) {
    const connectorTypes = new Set();
    for (const key of connectorMetrics.keys()) {
        const [type] = key.split(':');
        connectorTypes.add(type);
    }
    return Array.from(connectorTypes).map(type => getConnectorHealthStats(type, undefined, timeRangeMs));
}
/**
 * Get connector health check status
 */
export function getConnectorHealthCheck(connectorType, connectorId) {
...
//# sourceMappingURL=connector-health.js.map