 ...
 */
import { checkAlertPolicies, getActiveAlerts, DEFAULT_ALERT_POLICIES } from './alerts';
import { deliverAlert, loadAlertDeliveryConfig } from './alert-delivery';
 ...
 */
export async function checkAndDeliverAlerts(config, policies = DEFAULT_ALERT_POLICIES) {
 ...
 */
export function startPeriodicAlertChecking(intervalMs = 60 * 1000, // 1 minute default
config, policies) {
 ...
 */
export async function testAlertDelivery(config) {
    const deliveryConfig = config || loadAlertDeliveryConfig();
    const testAlert = {
        policyId: 'test_alert',
        severity: 'info',
    ...
    const testPolicy = {
        ...
        channels: ['slack', 'email', 'pagerduty', 'webhook'].filter(channel => {
            ...
            if (channel === 'slack')
                return !!deliveryConfig.slack;
            if (channel === 'email')
                return !!deliveryConfig.email;
            if (channel === 'pagerduty')
                return !!deliveryConfig.pagerduty;
            if (channel === 'webhook')
                return !!deliveryConfig.webhook;
            return false;
        }),
        condition: () => true,
