 ...
 */
export function recordErrorMetric(metric) {
    const key = `${metric.endpoint}:${metric.method}`;
 ...
 */
export function getPerformanceStats(timeRangeMs = 60 * 60 * 1000, // 1 hour default
endpoint) {
    const now = Date.now();
 ...
 */
export function getResourceStats(timeRangeMs = 60 * 60 * 1000) {
    const now = Date.now();
    const startTime = now - timeRangeMs;
 ...
 */
export function getAllErrorMetrics() {
...
let resourceMonitoringInterval = null;
/**
 * Start periodic resource monitoring
 */
export function startResourceMonitoring(intervalMs = 30000) {
