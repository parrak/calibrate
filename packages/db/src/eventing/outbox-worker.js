...
export class OutboxWorker {
    ...
    async processEvent(outboxId) {
        ...
        try {
            ...
            const matchingSubscribers = this.subscribers.filter((sub) => sub.eventTypes.includes(outboxEvent.eventType));
            ...
            for (const subscriber of matchingSubscribers) {
                await subscriber.handler({
                    ...
                    metadata: eventLog.metadata || undefined,
            ...
            if (newRetryCount >= outboxEvent.maxRetries) {
                ...
                console.log(`[OutboxWorker] Scheduled retry ${newRetryCount}/${outboxEvent.maxRetries} for event ${outboxId} at ${nextRetryAt.toISOString()}`);
            }
        }
    }
    /**
     ...
     */
    async moveToDeadLetterQueue(outboxId, errorMessage, retryCount) {
        try {
            ...
            console.error(`[OutboxWorker] Moved event ${outboxId} to DLQ after ${retryCount} retries: ${errorMessage}`);
        }
        catch (error) {
            console.error(`[OutboxWorker] Failed to move event to DLQ:`, error instanceof Error ? error.message : 'Unknown error');
        }
    }
    /**
     * Calculate next retry time with exponential backoff
