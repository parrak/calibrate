...
export class EventReplay {
    ...
    async replay(options) {
        ...
        for (const event of events) {
            ...
            const matchingSubscribers = this.subscribers.filter((sub) => sub.eventTypes.includes(event.eventType));
            ...
            for (const subscriber of matchingSubscribers) {
                try {
                    ...
                    console.error(`[EventReplay] Error replaying event ${event.id}:`, error instanceof Error ? error.message : 'Unknown error');
     ...
     */
    async getEventStats(tenantId, eventType) {
        const where = {
            tenantId,
            ...(eventType && { eventType })
        };
